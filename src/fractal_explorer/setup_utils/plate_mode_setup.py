...
def extras_from_url(plate_url: str) -> dict[str, str]:
    """Get the extras from the URL."""
    *_, plate_name, row, column, path_in_well = plate_url.split("/")
    return {
...
def build_plate_setup_df(urls: list[str], token=None) -> pl.DataFrame:
    plates = []
    for url in urls:
...
def _plate_name_selection_widget(plate_setup_df: pl.DataFrame) -> list[str]:
    """Create a widget for selecting plates."""
    plates = plate_setup_df["plate_name"].unique().sort().to_list()
    plates_names = pills_component(
...
def plate_name_selection(
    ...
    """Create a widget for selecting plates."""
    plates_names = _plate_name_selection_widget(plate_setup_df)
    plate_setup_df = _plate_selection_filter(plate_setup_df, plates_names)
    return plate_setup_df
...
def _collect_plate_condition_tables(
    plate_setup_df: pl.DataFrame, token=None
) -> list[str]:
    """Collect existing condition tables from the plate URLs."""
    plate_urls = plate_setup_df["plate_url"].unique().to_list()
    plate_condition_tables = set()

    for url in plate_urls:
        plate = get_ome_zarr_plate(url, token=token)
        try:
            plate_condition_tables.update(
                plate.list_image_tables(filter_types="condition_table")
            )
        except Exception as e:
            st.warning(f"Error loading condition tables: {e}")
            continue
    return list(plate_condition_tables)


def _collect_image_condition_tables(
    plate_setup_df: pl.DataFrame, token=None
) -> list[str]:
    """Collect existing condition tables from the image URLs."""
    ...
    images_condition_tables = asyncio.run(
        list_image_tables_async(images=images, filter_types="condition_table")
    )
...
def join_condition_tables(
    ...
    """Join the condition table with the plate setup DataFrame."""
    table_condition_tables = _collect_plate_condition_tables(
        plate_setup_df, token=token
    )
    image_condition_tables = _collect_image_condition_tables(
        plate_setup_df, token=token
    )
    selected_table, mode = _join_condition_table_widget(
        table_condition_tables, image_condition_tables
    )
...
def _collect_plate_features_tables(
    plate_setup_df: pl.DataFrame, token=None
) -> list[str]:
    """Collect existing feature tables from the plate URLs."""
    plate_urls = plate_setup_df["plate_url"].unique().to_list()
    plate_features_tables = set()

    for url in plate_urls:
        plate = get_ome_zarr_plate(url, token=token)
        try:
            plate_features_tables.update(
                plate.list_image_tables(filter_types="feature_table")
            )
        except Exception as e:
            st.warning(f"Error loading condition tables: {e}")
            continue
    return list(plate_features_tables)


def _collect_image_features_tables(
    plate_setup_df: pl.DataFrame, token=None
) -> list[str]:
    """Collect existing feature tables from the image URLs."""
    images_urls = plate_setup_df["image_url"].unique().to_list()
    images = [
        get_ome_zarr_container(url, token=token, mode="plate") for url in images_urls
    ]
    image_feature_condition_tables = asyncio.run(
        list_image_tables_async(images=images, filter_types="feature_table")
    )
    return image_feature_condition_tables
...
def load_feature_table(
    ...
    """Load the feature table from the plate URLs."""
    plate_feature_tables = _collect_plate_features_tables(plate_setup_df, token=token)
    image_feature_tables = _collect_image_features_tables(plate_setup_df, token=token)
...
def plate_mode_setup():
    ...
    if st.button("Add Plate URL"):
        ...
        try:
            ...
            st.session_state[f"{Scope.SETUP}:plate_setup:urls"].add(new_url)
        except Exception as e:
            st.error(f"Invalid URL: {e}")

    for user_url in st.session_state[f"{Scope.SETUP}:plate_setup:urls"]:
    ...
    with st.expander("Single Well Selection", expanded=False):
